...
Logs displays application logs for the selected environment. You can access the logs by navigating to
`Application > Environment > Logs`. Logs are refreshed every `10 seconds`, so you can see the latest logs.
...
In the logs screen, click on `Configure Log Levels` button to configure the log levels.
